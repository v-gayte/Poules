                ...
                return (
                  <div
                    ...
                    className={`bg-gray-900/50 p-2 rounded border border-dashed border-gray-700 flex flex-col items-center justify-center gap-1 min-h-[80px] group cursor-pointer hover:bg-gray-800 relative ${
                      basePc.techReq && !store.unlockedTechs.includes(basePc.techReq) ? 'opacity-50 pointer-events-none' : ''
                    }`}
                  ...
                  if (slot) {
                    ...
                    return (
                      <div
                        key={index}
                        className={`bg-gray-900/50 p-2 rounded border border-dashed border-gray-700 flex flex-col items-center justify-center gap-1 min-h-[80px] group cursor-pointer hover:bg-gray-800 relative ${
                           baseNetwork.techReq && !store.unlockedTechs.includes(baseNetwork.techReq) ? 'opacity-50 pointer-events-none' : ''
                        }`}
                  ...
                  if (slot) {
                    ...
                    return (
                      <div
                        key={index}
                        className={`bg-gray-900/50 p-2 rounded border border-dashed border-gray-700 flex flex-col items-center justify-center gap-1 min-h-[80px] group cursor-pointer hover:bg-gray-800 relative ${
                           baseTeacher.techReq && !store.unlockedTechs.includes(baseTeacher.techReq) ? 'opacity-50 pointer-events-none' : ''
                        }`}
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-700 flex z-20 h-[500px] overflow-hidden shadow-2xl">
        ...
        <div className="w-1/4 border-r border-gray-700 bg-gray-800 p-6 overflow-y-auto flex flex-col gap-4 shadow-lg z-10">
          ...
          <div className="bg-gray-900/80 p-5 rounded-xl border border-gray-600/50 backdrop-blur-sm">
            <div className="text-xs uppercase tracking-wider text-gray-500 font-bold mb-1">Research Output</div>
            <div className="text-4xl font-bold text-purple-400 drop-shadow-lg">+{config.rpGeneration} <span className="text-lg text-purple-500/80">RP/s</span></div>
          </div>
          ...
          {nextConfig ? (
            <div className="mt-auto">
              <div className="text-xs text-gray-400 mb-2 flex justify-between">
                 ...
                 <span className="text-green-400 font-bold">${(nextConfig.cost * (1 - store.globalModifiers.costReduction)).toLocaleString()}</span>
              </div>
              <button
            ...
            {TECH_TREE.map((tech) => {
              ...
              return (
                <div
                  ...
                  `}
                >
                  <div className="text-[10px] font-bold text-gray-500 uppercase">{tech.category}</div>
                  <div className={`font-bold text-xs leading-tight ${isUnlocked ? 'text-white' : 'text-gray-300'}`}>
                    {tech.name}
                  </div>
                  
                  {!isUnlocked && (
                    <div className={`text-[10px] font-mono mt-1 ${canUnlock ? 'text-purple-300' : 'text-gray-500'}`}>
                      {tech.cost} RP
                    </div>
                  )}
        ...
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white z-50 bg-gray-900/50 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
          ✕
        </button>
      </div>
    )
                      ...
                      {Object.values(SERVER_ASSETS).map((asset) => (
                        <button
                          ...
                          disabled={
                            ...
                            !!(asset.techReq && !store.unlockedTechs.includes(asset.techReq))
                          }
                          className="px-2 py-1 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 text-[10px] rounded text-left truncate relative"
